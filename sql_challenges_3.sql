...
SELECT MAX(qty) AS max_books_sold FROM sales;

-- 5. What's the price of the cheapest book?

SELECT MIN(price) AS cheapest_book_price FROM titles;
...
SELECT SUM(ytd_sales) AS total_ytd_sales FROM titles;

 -- 8. What's the average price of books?

SELECT AVG(price) AS average_book_price FROM titles;

/* 9. In a single query, select the count, average and sum of quantity in the
table sales */

SELECT COUNT(qty) AS qty_count, AVG(qty) AS average_qty, SUM(qty) AS total_qty FROM sales;